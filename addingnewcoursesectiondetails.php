    ...
    if(strcmp($faculty_department_id, $course_department_id) == 0) 
    {
        ...
        if( $course_section_faculty_count<2)
        {
            // run insert statement
                ...
                $sql=mysqli_query($conn,"SELECT max(crn) FROM course_section;");
                ...
                $sql_faculty_history = "INSERT INTO faculty_history(courseid,facultyid,crn,semesterid) values('$courseid','$facultyid','$newcrn','$semesterid')";
                $res_faculty_history = mysqli_query($conn1, $sql_faculty_history) or die(mysqli_error($conn1));

                echo "NEW COURSE SECTION ADDED!";
        }
        else
        {
