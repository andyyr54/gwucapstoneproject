...
$sql_student_history_exists=mysqli_query($conn,"SELECT * FROM student_history where studentid='$userid' ");
          $row_student_history_exists= mysqli_fetch_array($sql_student_history_exists);
          if(is_array($row_student_history_exists))
          {

            $sql_student_history_semesterid = "SELECT DISTINCT(semesterid) FROM student_history where studentid='$userid' ORDER BY semesterid DESC;";
            $row_student_history_semesterid= mysqli_query($conn1, $sql_student_history_semesterid) or die(mysqli_error($conn1));
           
            //looping through each dayid and getting details relevant
            while($det_student_history_semesterid = mysqli_fetch_assoc($row_student_history_semesterid))
           {
            // echo "<hr><hr>";
           $semesterid=$det_student_history_semesterid['semesterid'];
            ?>
    <div class="container">
...
$studentid=$userid;
 ...
 while($det_student_history = mysqli_fetch_assoc($row_student_history)){
  ...
  $grade_obtained=$det_student_history['grade'];
          ...
          {
            $link="updategrade.php?crn=".$crn."&studentid=".$userid;
...
$row_course= mysqli_fetch_array($sql_course);
 ...
 $number_of_credits=$row_course['no_of_credits'];
            ...
            <td><?php echo $course_name; ?></td>
                ...
                <td><a href="<?php echo $link ?>"><button type="button" >Update Grade</button></a></td>
          <?php
...
{
    ?>
<h2>There is no Transcript present as Student haven't enrolled to any courses in this semester</h2>
<?php
}
?>
